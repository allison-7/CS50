...
-- Get all the tables and variables in the database
.schema
-- Checking the description of the crime
SELECT description
...
-- Two incidents happened. One is related to theft. The other is related to littering.
-- The witness could be the accomplice. So, finding the names of the witnesses from the interviews table.
SELECT name, transcript
  FROM interviews
 WHERE year = 2021
   AND month = 7
   AND day = 28;
-- Found two transcripts with the name Eugene Check how many Eugenes are in the 'people' table.
SELECT name
  FROM people
 WHERE name = 'Eugene';
...
-- Thief was withdrawing money from the ATM on Leggett Street. Check the account number of the person who did that transaction.
...
-- Raymond said as they were leaving the bakery, they called a person and talked for less than a minute. They asked the person on the other end of the call to buy a flight ticket of the earliest flight on July 29, 2021.
...
-- Ordering them according to the durations of the calls.
...
-- Checking the possible names of the call-receiver. Ordering them according to the durations of the calls.
SELECT name, phone_calls.duration
  FROM people
  JOIN phone_calls
 ...
 WHERE phone_calls.year = 2021
   ...
   AND phone_calls.duration <= 60
   ORDER BY phone_calls.duration;
-- The thief drove away in a car from the bakery, within 10 minutes from the theft. Check the license plates of cars within that time frame.
-- Then, checking out the names of those cars' owners.
...
-- Bruce must the thief as he is present in all the 4 lists.
